...
bibliography: ../../APA-Style/bibliographies/RepResRef.bib
vignette: >
...
Beide Tests sind die gleichen Testverfahren, die genaue Bezeichnung ist Wilcoxon-Mann-Whitney-Test und auch der H-Test ist mathematisch dasselbe Verfahren [@BortzLienert] (Seite 140 und 203). (Die Verwechslung rührt wahrscheinlich daher, dass noch der Wilcoxon-Vorzeichenrangtest als Verfahren existiert.)
...
```{r fig-roc1, fig.cap = "ROC-Kurve zu den Blutzuckerwerten",  fig.width=4, fig.height=4, cache=TRUE}
# require(pROC)
...
```{r fig-roc2, fig.cap = "ROC-Kurve zu den Harnblasenkarzinom",  fig.width=4, fig.height=4, cache=TRUE}
fit1<- glm(gruppe~lai, hkarz, family = binomial)
...
```{r, Giavarina-data, include=FALSE}
set.seed(0815)
...
Kreuztabelle: Cohen’s w, φ, Cramer’s V, C
ANOVA/Regression: f, η², η²part und d, Δ, Hedge’s g (bei Vergleich der Stichproben) sowie Odds Ratio, Risk Ratio, r, R²
...
Der RMSE ist eine relative Grösse (in der Einheit der Zielvariablen), je kleiner umso besser.
...
Aber die Ergebnisse sind *anderst* als bei PSPP und bei meiner Funktion?? - Gehoert also noch ueberprueft.
...
Zum Vergleich von zwei Therapien werden
häufig Überlebenszeiten herangezogen. Diese sollten 
...
Die Datei mkarz.sav Seite 553 ist ein Datensatz mit 106 Patientan
...
Erkennen on Autokorrelation   Der DW d Test weist folgendes Intervall auf:
...
Multikollinearität beschreibt einen Sachverhalt, i d i kläd Vibl i in dem eine erklärende Variable eine Linearkombination einer anderen oder mehrerer anderer erklärender Variablen ist.
...
mit Eigenwerten ≥ 1 wurden in Betracht gezogen (Guttman, 1954; Kaiser, 1960).
